...
var queueRunningLimit = 20;
var warningQueueLength = queueRunningLimit << 1;
...
/**
 * [arrange 将数据整理（合并相同数据，生成minutes tenMinutes hours字段）]
 * @param {[type]} cbf
...
var update = function(collection, conditions, data){
  if(!~collection.indexOf('stats-')){
    ...
    if(queueLength > warningQueueLength){
      console.log('update queue running:%d, length:%d', updateQueue.running(), queueLength);
    }
